// Copyright (c) 2014 The MoneyCoin developers
...
MoneyGUI::MoneyGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new MoneyBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("MoneyClient") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/money"));
...
#ifdef Q_OS_WIN
    QFile html("C:/money/index.html");
#else
    QFile html("/opt/money/index.html");
#endif
...
void MoneyGUI::pageLoaded(bool ok)
{
...
void MoneyGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void MoneyGUI::urlClicked(const QUrl & link)
{
...
void MoneyGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/money"), tr("&About MoneyCoin"), this);
...
void MoneyGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("MoneyClient") + " - " + tr("Wallet") + ", " + sMode);
        };
        ...
        if (clientModel->isTestNet())
        {
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/money_testnet"));
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("MoneyClient client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/money_testnet"));
...
void MoneyGUI::setMessageModel(MessageModel *messageModel)
{
...
void MoneyGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("MoneyClient client"));
    trayIcon->setIcon(QIcon(":/icons/money"));
...
void MoneyGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to MoneyCoin network", "", count));
}
...
void MoneyGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
    ...
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          ...
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::MONEY, nFeeRequired));
...
void MoneyGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void MoneyGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}
...
void MoneyGUI::updateStakingIcon()
{
