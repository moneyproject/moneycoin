...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return MoneyGUI::tr("Received with");
    case RecvFromOther:
        return MoneyGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return MoneyGUI::tr("Sent to");
    case SendToSelf:
        return MoneyGUI::tr("Payment to yourself");
    case Generated:
        return MoneyGUI::tr("Mined");
    case RecvMoney:
        return MoneyGUI::tr("Received money");
    case SendMoney:
        return MoneyGUI::tr("Sent money");
    case Other:
        return MoneyGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvMoney:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendMoney:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredMONEY, nCredMoney;
    wtx.GetCredit(nCredMONEY, nCredMoney, true);
    int64_t nCredit = nCredMONEY + nCredMoney;
    ...
    {
        if (nNet > 0 && nCredMoney > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvMoney, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendMoney, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvMoney;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv money
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendMoney;
                    sub.address = CBitcoinAddress(ckidD).ToString();
