...
#include "moneybridge.moc"

MoneyBridge::MoneyBridge(MoneyGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void MoneyBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void MoneyBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void MoneyBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void MoneyBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void MoneyBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void MoneyBridge::populateOptions()
{
...
// Transactions
bool MoneyBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void MoneyBridge::clearRecipients()
{
    recipients.clear();
}

bool MoneyBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 MONEY, 1 Money
        switch(rcp.txnTypeInd)
        {
            case TXT_MONEY_TO_MONEY:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MONEY, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_MONEY_TO_ANON:
                formatted.append(tr("<b>%1</b> to MONEY %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MONEY, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> MONEY, ring size %2 to MONEY %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MONEY, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_MONEY:
                formatted.append(tr("<b>%1</b> MONEY, ring size %2 to MONEY %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MONEY, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MONEY, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsMoneyBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your money balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MONEY, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void MoneyBridge::openCoinControl()
{
...
void MoneyBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void MoneyBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap MoneyBridge::listAnonOutputs()
{
...
void MoneyBridge::populateTransactionTable()
{
...
void MoneyBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void MoneyBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString MoneyBridge::transactionDetails(QString txid)
{
...
// Addresses
void MoneyBridge::populateAddressTable()
{
...
void MoneyBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void MoneyBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString MoneyBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString MoneyBridge::lastAddressError()
{
    QString sError;
...
QString MoneyBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void MoneyBridge::updateAddressLabel(QString address, QString label)
{
...
bool MoneyBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool MoneyBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void MoneyBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void MoneyBridge::appendMessage(int row)
{
...
void MoneyBridge::populateMessageTable()
{
...
void MoneyBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool MoneyBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool MoneyBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString MoneyBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString MoneyBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool MoneyBridge::setPubKey(QString address, QString pubkey)
{
...
bool MoneyBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString MoneyBridge::createGroupChat(QString label)
{
...
QString MoneyBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList MoneyBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void MoneyBridge::connectSignals()
{
...
QString MoneyBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap MoneyBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap MoneyBridge::listLatestBlocks()
{
...
QVariantMap MoneyBridge::findBlock(QString searchID)
{
...
QVariantMap MoneyBridge::blockDetails(QString blkHash)
{
...
QVariantMap MoneyBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap MoneyBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap MoneyBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap MoneyBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap MoneyBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap MoneyBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap MoneyBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap MoneyBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap MoneyBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Moneycoin prefix.");
            return result;
        }
...
QVariantMap MoneyBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap MoneyBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap MoneyBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
