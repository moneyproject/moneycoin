...
public:
    ...
    int64_t GetMoneyDebit(const CTxIn& txin) const;
    int64_t GetMoneyCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nMONEY, int64_t& nMoney) const
    {
        nMONEY = 0;
        nMoney = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nMoney += GetMoneyCredit(txout);
            } else
                nMONEY += GetCredit(txout);
            if (!MoneyRange(nMONEY)
                || !MoneyRange(nMoney))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
...
public:
    ...
    mutable bool fAvailableMoneyCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableMoneyCreditCached;
    
    mutable int64_t nCredMONEYCached;
    mutable int64_t nCredMoneyCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableMoneyCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredMONEY, int64_t& nCredMoney, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredMONEY = nCredMoney = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredMONEYCached = 0;
            nCredMoneyCached = 0;
            pwallet->GetCredit(*this, nCredMONEYCached, nCredMoneyCached);
            fCreditSplitCached = true;
        };
        
        nCredMONEY = nCredMONEYCached;
        nCredMoney = nCredMoneyCached;
        return true;
    }
    ...
    int64_t GetAvailableMoneyCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableMoneyCreditCached)
            return nAvailableMoneyCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableMoneyCredit() : value out of range");
            };
        };
        
        nAvailableMoneyCreditCached = nCredit;
        fAvailableMoneyCreditCached = true;
        return nCredit;
    };
